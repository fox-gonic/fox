...
Fox 目前处于 `0.1.x` 阶段，正在积极开发中。公开 API 正在趋于稳定，但仍可能在小版本之间引入破坏性变更——每一项都会在 [CHANGELOG.md](../CHANGELOG.md) 中明确列出并附带迁移说明。在了解这一点的前提下可以用于生产环境；建议固定到具体小版本，并在升级前查阅 changelog。
